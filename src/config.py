...
# --- 配置数据类 ---
class Config:
    def __init__(self, data: Union[Dict[str, Any], tomlkit.TOMLDocument]):
...
def _merge_toml_data(new_data, old_data):
        ...
        if key in new_data and isinstance(old_data[key], type(new_data[key])):
            if isinstance(new_data[key], tomlkit.items.Table):
                for sub_key in old_data[key]:
                    if sub_key in new_data[key] and isinstance(old_data[key][sub_key], type(new_data[key][sub_key])):
...
def _init_config() -> bool:
    ...
    backup_path = BACKUP_DIR / f"config_v{actual_version}_{datetime.datetime.now().strftime('%Y%m%d%H%M%S')}.toml"
