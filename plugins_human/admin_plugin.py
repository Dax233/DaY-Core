...
async def handle_kick(adapter: Adapter, event: GroupMessageEvent):
    # 简单的权限检查
    ...
    if result is not API_FAILED:
        ...
        await adapter.send_message(event.group_id, "group", Message.text(f"已将用户 {user_to_kick} 移出本群。"))
    else:
        # 只有当 result 确确实实是 API_FAILED 时，才算失败
        await adapter.send_message(event.group_id, "group", Message.text(f"操作失败，可能是我没有权限，或者发生了未知错误。"))